...
{
    if (query_.isVerbose()) {
        qDebug()<<"\t  ["<<loadingType_<<"] Package \""<<repositoryInfo.getUrl()+"/"+requireInfo.getPackagePath()<<"\" available ?";
    }
    return QDir(repositoryInfo.getUrl()+"/"+requireInfo.getPackagePath()).exists();
}
...
bool Qompoter::FsLoader::load(const PackageInfo &packageInfo) const
{
    ...
    if (query_.isVerbose()) {
        qDebug()<<"\t  ["<<loadingType_<<"] Load package source \""<<packageSourcePath<<"\"";
        qDebug()<<"\t  ["<<loadingType_<<"] To package dest \""<<packageDestPath<<"\"";
    }
